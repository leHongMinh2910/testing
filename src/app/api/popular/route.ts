...
/**
 * GET /api/popular
 * Get popular books from Gorse API
 */
export async function GET(request: NextRequest) {
  try {
    ...
    // Call Gorse popular items API
    const popularItems = await GorseService.getPopularItems(n, category);
    ...
    // Maintain order from Gorse popular items (by Score, descending)
    // Create a map of book IDs to their scores
    const scoreMap = new Map(
      popularItems
        .map(item => {
          const bookId = GorseService.fromGorseItemId(item.Id);
          return bookId !== null ? [bookId, item.Score] : null;
        })
        .filter((item): item is [number, number] => item !== null)
    );

    // Sort books by score (descending), then by the order in popularItems
    const bookMap = new Map(books.map(book => [book.id, book]));
    const orderedBooks = popularItems
      .map(item => {
        const bookId = GorseService.fromGorseItemId(item.Id);
    ...
    return handleRouteError(error, 'GET /api/popular');
  }
}
